...
class SimulationConfig:
    ...
    fog_density:    float = 0.0            # g/m³; only used if model == "fog"
    fog_liquid_water_density_gm3: float = 0.05
    fog_visibility_km: float = 1.0
...
def _apply_dict_to_config(cfg: SimulationConfig, d: dict) -> SimulationConfig:
    ...
    aliases = {
        ...
        "fog_visibility": "fog_visibility_km",
    ...
    float_fields = {
        ...
        "fog_density", "fog_liquid_water_density_gm3", "fog_visibility_km",
    ...
    for k, v in d.items():
        ...
        try:
            if key in bool_fields:
                setattr(cfg, key, str(v).lower() in ("1", "true", "yes"))
...
def _from_csv(path: str) -> SimulationConfig:
    ...
    if len(rows) == 1:
        ...
        # Treat as receiver grid: columns must include lat, lng [, height_m]
        cfg = SimulationConfig()
        receivers = []
        for row in rows:
            try:
                rx = ReceiverPoint(
                    ...
                    height_m=float(row.get("height_m", 1.5))
                )
                receivers.append(rx)
